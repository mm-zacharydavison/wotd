...
install_wotd() {
    ...
    echo -e ">> What is your native language? (e.g. 'English')\e[0m"
    echo -e ">> Meanings of words will be shown in your native language.\e[0m"
    ...
    echo -e ">> Which languages do you want get words of the day for? (comma-separated, e.g., English,Spanish,French)\e[0m"
    read -r ENABLED_LANGUAGES

    # Check if both API keys are set
    if [ -n "$OPENAI_API_KEY" ] && [ -n "$ANTHROPIC_API_KEY" ]; then
        # Ask the user which AI service they prefer
        echo -e ">> Both OpenAI and Claude API keys are set. Which AI service do you prefer? (openai/claude)\e[0m"
        read -r PREFERRED_AI
        ...
        case "$PREFERRED_AI" in
            ...
            *) echo -e ">> Invalid choice. Defaulting to OpenAI.\e[0m" && PREFERRED_AI="openai" ;;
        esac
        ...
        echo -e ">> Error: Neither OPENAI_API_KEY nor ANTHROPIC_API_KEY is set. Please set at least one in your environment variables.\e[0m"
        exit 1
    fi
    ...
    echo -e ">> Adding the Word of the Day script to your shell configuration files...\e[0m"
    for config_file in "$BASHRC" "$ZSHRC" "$FISH_CONFIG"; do
        if [ -f "$config_file" ]; then
            if ! grep -q "$INSTALL_DIR/$SCRIPT_NAME" "$config_file"; then
                ...
                echo -e ">> Added to $config_file.\e[0m"
            else
                echo -e ">> Already added to $config_file.\e[0m"
            fi
        fi
    done

    echo -e ">> Installation complete. Please restart your shell to see the Word of the Day.\e[0m"
}
